...
<div class="p-4 pb-24 sm:p-6">
	...
	{:else if album}
		<!-- Album Header -->
		<div class="mb-8 flex flex-col items-center gap-6 sm:flex-row sm:items-start">
			<div class="h-48 w-48 flex-shrink-0 overflow-hidden rounded-lg bg-surface shadow-lg">
				<img
			...
			<div class="text-center sm:text-left">
				...
				<div class="mt-4 flex items-center justify-center gap-3 sm:justify-start">
					<button
						onclick={() => playAll()}
						class="flex items-center gap-2 rounded-full bg-accent px-5 py-2 text-sm font-medium text-accent-text transition-colors hover:bg-accent-hover"
					>
					...
					<button
						...
						class="rounded-full border border-border p-2.5 transition-colors hover:bg-surface {album.UserData?.IsFavorite ? 'text-accent' : 'text-text-muted'}"
						aria-label="Favorite"
					>
